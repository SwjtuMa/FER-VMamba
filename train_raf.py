...
from FER_VMamba import MixedFeatureNet
...
def parse_args():
    ...
    parser.add_argument('--raf_path', type=str, default='/workspace/fermamba/raf/original_100/', help='Raf-DB dataset path.')
...
def run_training():
    ...
    model = MixedFeatureNet()  
    model = model.cuda()    
    device = next(model.parameters()).device  
    pretrained_state_dict = torch.load('/workspace/fermamba/raf/pretrained/MFN_msceleb.pth')  
    ...
    for epoch in tqdm(range(1, args.epochs + 1)):
        ...
        for (imgs, targets) in train_loader:
            ...
            output, region_preds = model(imgs)
            att_wt = 0.7
            loss = att_wt*criterion_cls(output,targets) + (1-att_wt)* criterion_cls(region_preds,targets)        
            optimizer.zero_grad()
            loss.backward()
            ...
            output, region_preds = model(imgs)
            loss = att_wt*criterion_cls(output,targets) + (1-att_wt)* criterion_cls(region_preds,targets)
            out = att_wt*output + (1-att_wt) * region_preds
            optimizer.zero_grad()
            loss.backward()
